...
export default class LinkedList {
    ...
    reverse(){
        ...
        // o antigo head vira o novo tail
        this.#tail = this.#head

        while (current !== null) {
            next = current.next     // guarda o próximo
            current.next = prev     // inverte o ponteiro
            prev = current          // avança o prev
            current = next          // avança o current
        }

        // o último vira o novo head
        this.#head = prev
    }
    ...
    removeAll(val) {
        ...
        while(current !== null) {
            ...
            if(current.data === val) {
                ...
                // Caso 2 e 3: meio ou final
                else {
                    before.next = current.next

                    // Se for o último, atualiza o tail
